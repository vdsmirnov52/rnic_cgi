#!/usr/bin/python
# Calculate CRC16 check sum for binary file
# (c) Alexander Belchenko, 2005
# e-mail: bialix@ukrpost.net
# v.1.0.20050413

'''
Calculate CRC16 check sum for binary file
Author: Alexander Belcenko, 2005

Usage:
    python crc16.py [options] infile [outfile]

Options:
    -q, -Q  --  quiet run
    -r      --  write checksum to last 2 bytes of input file
    -a      --  append checksum to input file
    -l      --  write checksum to file in little Endian form
    -b, -B  --  write checksum to file in Big Endian form (default)

Without options -r or -a simply prints calculated sum.
For calculations uses polynom 0x1021 with initial value of CRC as 0x0000.

By default utility calculate checksum for all bytes excerpt last 2.
When specified options -a then checksummed all bytes in file.
'''

#-----------------------------------------------------------------------------
import sys, os

#-----------------------------------------------------------------------------
# internal functions and data for crc16 calculation
crc16htab = ( \
            )

crc16ltab = ( \
            )

global crc_h, crc_l

def crc_live(b):
    global crc_h, crc_l
    byte = ord(b)
    index = crc_h ^ byte
    crc_h = crc_l ^ crc16htab[index]
    crc_l =         crc16ltab[index]

def crc_prepare():
    global crc_h, crc_l
    crc_h = crc_l = 0
    
def crc_result():
    global crc_h, crc_l
    return ( crc_h << 8 ) | crc_l

#-----------------------------------------------------------------------------
options = {'r': False, 'a': False, 'q': False, 'l': False, 'b': False}
files = []

if __name__ == '__main__':
    if len(sys.argv) == 1:
        print __doc__
        sys.exit(1)

    # processing of options
    opt_flag = True

    for i in sys.argv[1:]:
        if i.startswith('-'):
            if not opt_flag:
                print 'Error: options after filename not allowed'
                sys.exit(2)
            else:
                o = i[1:].lower()
                if o in options.keys():
                    options[o] = True
                else:
                    print 'Error: unrecognized options <%s>' % i
                    sys.exit(2)
                if options['r'] == True and options['a'] == True:
                    print 'Error:',
                    print 'options -r and -a will not be used simultaneously'
                    sys.exit(2)
                if options['l'] == True and options['b'] == True:
                    print 'Error:',
                    print 'options -l and -b will not be used simultaneously'
                    sys.exit(2)
        else:
            opt_flag = False
            if len(files) < 2:
                files.append(i)
            else:
                print 'Error: too much options in command line'
                sys.exit(2)

    szf = len(files)
    if szf == 0:
        print 'Error: specify name of input file'
        sys.exit(2)
    elif szf == 1:
        files = files * 2

    # processing of files
    try:
        f = file(files[0], 'rb')
        data = f.read()
        f.close()
    except IOError:
        print 'File <%s> does not exists' % files[0]
        sys.exit(1)

    size = len(data)
    if size < 3:
        print 'Warning: file size less than 3 bytes'
        sys.exit(0)
    if not options['a']:
        size -= 2

    crc_prepare()

    for i in xrange(size):
        crc_live(data[i])
    
    crc = crc_result()

    if not options['q']:
        print 'CRC: 0x%04X' % crc


    if options['r'] or options['a']:
        if options['l'] == True:
            crcs = '%c%c' % (chr(crc_l), chr(crc_h))
        else:
            crcs = '%c%c' % (chr(crc_h), chr(crc_l))

        try:
            f = file(files[1], 'wb')
            f.write(data[:size])
            f.write(crcs)
            f.close()
        except IOError:
            print 'File error: could not write to file <%s>' % files[1]
            sys.exit(1)

    sys.exit(0)
